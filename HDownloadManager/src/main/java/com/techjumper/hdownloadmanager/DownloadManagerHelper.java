...
public class DownloadManagerHelper {
    ...
    private static void initPauseMethod() {
        ...
        try {
            pauseDownload = DownloadManager.class.getDeclaredMethod(METHOD_NAME_PAUSE_DOWNLOAD, long[].class);
    ...
    private static void initResumeMethod() {
        ...
        try {
            resumeDownload = DownloadManager.class.getDeclaredMethod(METHOD_NAME_RESUME_DOWNLOAD, long[].class);
    ...
    public String getUrl(long downloadId) {
        return getString(downloadId, DownloadManager.COLUMN_DESCRIPTION);
    }
    ...
    public void removeAll() {
        ...
        try {
            c = mDownloadManager.query(query);
            while (c.moveToNext()) {
                int downloadId = c.getInt(c.getColumnIndex(DownloadManager.COLUMN_ID));
            ...
            Log.d("HIDETAG", "清空之前的下载: " + count + "个");
        }
    }

    public void removeAll(ArrayList<String> urls) {
        DownloadManager.Query query = new DownloadManager.Query();
        Cursor c = null;
        int count = 0;
        try {
